...
void transfer_list_dump(struct transfer_list_header *tl)
{
	...
	DEBUG ((DEBUG_INFO | DEBUG_LOAD,"size       0x%x\n", tl->size));
	DEBUG ((DEBUG_INFO | DEBUG_LOAD,"max_size   0x%x\n", tl->max_size));
...
transfer_list_check_header(const struct transfer_list_header *tl)
{
	...
	if (!tl->max_size) {
		DEBUG ((DEBUG_ERROR,"Bad transfer list max size 0x%x\n", tl->max_size));
		return TL_OPS_NON;
	}

	if (tl->size > tl->max_size) {
		DEBUG ((DEBUG_ERROR,"Bad transfer list size 0x%x\n", tl->size));
		return TL_OPS_NON;
	}
	...
	DEBUG ((DEBUG_INFO | DEBUG_LOAD,"Old transfer list version is detected\n"));
	return TL_OPS_CUS;
}
